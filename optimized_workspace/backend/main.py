...
async def analyze_text(payload: TextPayload, db: Session=Depends(get_db)):
    ...
    log_entry = AnalysisLog(content_type='text', content=payload.text, is_threat=result.get('is_threat'), risk_score=result.get('risk_score'), threat_type=result.get('threat_type'), message=result.get('message'))
    db.add(log_entry)
    db.commit()
    return result
...
async def analyze_url(payload: UrlPayload, db: Session=Depends(get_db)):
    ...
    if settings.strict_mode and result.get('risk_score', 0) > settings.alert_threshold:
        result['is_threat'] = True
    log_entry = AnalysisLog(content_type='url', content=payload.url, is_threat=result.get('is_threat'), risk_score=result.get('risk_score'), threat_type=result.get('threat_type'), message=result.get('message'))
    db.add(log_entry)
    db.commit()
    return result
...
async def analyze_image(payload: ImagePayload, db: Session=Depends(get_db)):
    ...
    if settings.strict_mode and result.get('risk_score', 0) > settings.alert_threshold:
        result['is_threat'] = True
    log_entry = AnalysisLog(content_type='image', content=payload.image_url, is_threat=result.get('is_threat'), risk_score=result.get('risk_score'), threat_type=result.get('threat_type', 'deepfake'), message=result.get('message'))
    db.add(log_entry)
    db.commit()
    return result
...
async def get_history(db: Session=Depends(get_db), username: str=Depends(auth.verify_token)):
    ...
    for log in logs:
        if not log.is_threat:
            ...
            t_type = 'warning'
        formatted_logs.append({'id': log.id, 'type': t_type, 'title': 'Threat Detected' if log.is_threat else 'Scan Clean', 'desc': log.message, 'time': log.timestamp.strftime('%H:%M:%S') if log.timestamp else ''})
...
async def get_dashboard_stats(db: Session=Depends(get_db), username: str=Depends(auth.verify_token)):
    ...
    return {'digital_twin_risk_score': random.randint(15, 45), 'threats_blocked_today': threats_blocked_today, 'dark_web_leaks_found': random.randint(0, 2), 'active_protections': 4}

@app.get('/api/settings')
async def get_api_settings(db: Session=Depends(get_db), username: str=Depends(auth.verify_token)):
    settings = get_settings(db)
    return {'strict_mode': settings.strict_mode, 'auto_block': settings.auto_block, 'alert_threshold': settings.alert_threshold}
